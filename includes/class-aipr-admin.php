...
class AIPR_Admin {
    ...
    const MAX_LIBRARY_BYTES = 1048576;
    const MAX_PATCH_UPLOAD_BYTES = 1048576;
    ...
    public static function required_capability() {
        /**
         * AI-PATCH: File-writing actions should require plugin update privileges, not only general options access.
         * On multisite this is restricted to super admins because plugin file changes affect the network.
         */
        if ( is_multisite() ) {
    ...
    private function can_manage_patches() {
    ...
    public function boot() {
        ...
        add_filter( 'option_page_capability_aipr_settings_group', array( $this, 'settings_capability' ) );
    ...
    private function get_notice() {
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only admin notice parameters generated by this plugin after nonce-protected redirects.
        if ( empty( $_GET['aipr_notice'] ) ) {
            return null;
        }

        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only admin notice message generated by this plugin after nonce-protected redirects.
        ...
        return array(
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only admin notice type generated by this plugin after nonce-protected redirects.
    ...
    private function set_library( $items ) {
        ...
        $items = array_slice( $items, - self::MAX_LIBRARY_ITEMS );

        while ( count( $items ) > 0 && strlen( wp_json_encode( $items ) ) > self::MAX_LIBRARY_BYTES ) {
            array_shift( $items );
        }

        update_option( self::LIBRARY_OPTION, $items, false );
    }
    ...
    private function current_slug() {
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- WordPress admin page routing parameter; read-only and sanitized.
    ...
    private function render_shell_start( $title, $eyebrow = '' ) {
        ...
        <div class="wrap aipr-wrap">
            ...
            <?php endif; ?>
            <?php // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Core Settings API success flag is read-only and sanitized. ?>
            ...
            <header class="aipr-topbar">
                <div>
                    ...
                    <div class="aipr-top-meta">
                        <span class="aipr-chip">
                            <?php
                            /* translators: %d: Number of detected plugins. */
                            echo esc_html( sprintf( __( '%d plugins gedetecteerd', 'ai-patch-runner' ), count( $plugins ) ) );
                            ?>
                        </span>
            ...
            <nav class="aipr-nav" aria-label="<?php echo esc_attr__( 'AI Patch Runner-secties', 'ai-patch-runner' ); ?>">
                ...
                $nav = array(
                    ...
                    /* translators: Placeholder(s) are replaced with dynamic plugin, file, size, count, or validation details. */
                        ...
                        <div class="aipr-stack aipr-space-top">
                            <?php foreach ( $grouped as $file_path => $checks ) : ?>
                                <article class="aipr-file-card">
                                    <div class="aipr-file-header"><div><span class="aipr-mini-label"><?php esc_html_e( 'Bestand', 'ai-patch-runner' ); ?></span><h3><code><?php echo esc_html( $file_path ); ?></code></h3></div><span class="aipr-chip"><?php
                                        /* translators: %d: Number of operations for this file. */
                                        echo esc_html( sprintf( _n( '%d operation', '%d operations', count( $checks ), 'ai-patch-runner' ), count( $checks ) ) );
                                    ?></span></div>
                                    <div class="aipr-stack">
                    ...
                    <?php foreach ( $library as $item ) : ?>
                        <article class="aipr-list-card">
                            <div class="aipr-list-card-head">
                                ...
                                <div class="aipr-chip-row"><span class="aipr-chip"><?php echo esc_html( $item['plugin'] ?? '' ); ?></span><span class="aipr-chip"><?php
                                    /* translators: %d: Number of operations in the saved patch. */
                                    echo esc_html( sprintf( _n( '%d operation', '%d operations', count( $item['package']['operations'] ?? array() ), 'ai-patch-runner' ), count( $item['package']['operations'] ?? array() ) ) );
                                ?></span></div>
                            </div>
    ...
    public function render_inspector_page() {
        ...
        $plugins  = $this->get_plugins_list();
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only inspector filter; sanitized and checked against get_plugins().
    ...
    public function sanitize_settings( $input ) {
        ...
        $constant_allows_production = defined( 'AIPR_ALLOW_PRODUCTION_APPLY' ) && true === AIPR_ALLOW_PRODUCTION_APPLY;

        return array(
            'allow_production_apply' => ( $constant_allows_production && ! empty( $input['allow_production_apply'] ) ) ? 1 : 0,
            'store_preview_library'  => empty( $input['store_preview_library'] ) ? 0 : 1,
            'cleanup_on_uninstall'   => empty( $input['cleanup_on_uninstall'] ) ? 0 : 1,
            'backup_retention_days'  => min( 365, max( 1, absint( $input['backup_retention_days'] ?? 30 ) ) ),
        );
    }

    public function render_settings_section_intro() {
        echo '<p class="aipr-muted">' . esc_html__( 'Productie is standaard dubbel vergrendeld. Zet dit alleen aan als AIPR_ALLOW_PRODUCTION_APPLY bewust op true staat en je het risico begrijpt.', 'ai-patch-runner' ) . '</p>';
    }
    ...
    public function render_allow_production_field() {
        ...
        <label class="aipr-toggle" for="aipr_allow_production_apply">
            <input id="aipr_allow_production_apply" type="checkbox" name="<?php echo esc_attr( self::SETTINGS_OPTION ); ?>[allow_production_apply]" value="1" <?php checked( ! empty( $settings['allow_production_apply'] ) && $constant_allows_production ); ?> <?php disabled( ! $constant_allows_production ); ?> />
            <span><?php esc_html_e( 'Patch toepassen op productieomgevingen toestaan wanneer AIPR_ALLOW_PRODUCTION_APPLY true is', 'ai-patch-runner' ); ?></span>
        </label>
        <?php if ( ! $constant_allows_production ) : ?>
            <p class="description aipr-muted"><?php esc_html_e( 'Server-side productie-apply is uitgeschakeld. Definieer AIPR_ALLOW_PRODUCTION_APPLY alleen tijdelijk en bewust als true.', 'ai-patch-runner' ); ?></p>
        <?php endif; ?>
        <?php
    }
    ...
    public function handle_preview() {
        ...
        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Temporary upload path is validated with is_uploaded_file(), is_readable(), and wp_check_filetype_and_ext() before use.
        ...
        if ( $uploaded_size > self::MAX_PATCH_UPLOAD_BYTES ) {
    ...
    public function handle_apply() {
        ...
        if ( 'production' === wp_get_environment_type() ) {
            $constant_allows_production = defined( 'AIPR_ALLOW_PRODUCTION_APPLY' ) && true === AIPR_ALLOW_PRODUCTION_APPLY;
            if ( ! $constant_allows_production || empty( $settings['allow_production_apply'] ) ) {
                $this->redirect_notice( 'error', __( 'Toepassen op productie is vergrendeld. Zet AIPR_ALLOW_PRODUCTION_APPLY op true én schakel de instelling bewust in.', 'ai-patch-runner' ), 'ai-patch-runner-settings' );
            }
        }
    ...
    public function handle_save_patch_library() {
        ...
        if ( strlen( wp_json_encode( $item ) ) > self::MAX_LIBRARY_BYTES ) {
            $this->redirect_notice( 'error', __( 'Deze patch is te groot om veilig in de bibliotheek op te slaan.', 'ai-patch-runner' ), 'ai-patch-runner-apply' );
        }
    ...
    private function is_safe_path_in_dir( $file_abs, $base_dir, $allow_missing ) {
        ...
        $normalized_file = wp_normalize_path( (string) $file_abs );

        if ( false !== strpos( $normalized_file, chr( 0 ) ) || false !== strpos( $normalized_file, '../' ) || false !== strpos( $normalized_file, '..\\' ) ) {
            return false;
        }

        if ( $allow_missing ) {
            $candidate_dir = dirname( $normalized_file );
            $existing_dir  = $candidate_dir;

            while ( ! is_dir( $existing_dir ) && dirname( $existing_dir ) !== $existing_dir ) {
                $existing_dir = dirname( $existing_dir );
            }

            $dir_real = realpath( $existing_dir );
            if ( false === $dir_real ) {
                return false;
            }

            $dir_path = trailingslashit( wp_normalize_path( $dir_real ) );
            if ( $base_path !== $dir_path && ! str_starts_with( $dir_path, $base_path ) ) {
                return false;
            }

            return str_starts_with( $normalized_file, $base_path );
        }
